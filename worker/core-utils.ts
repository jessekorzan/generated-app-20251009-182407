...
export abstract class Entity<State> {
  ...
  async save(next: State): Promise<void> {
    for (let i = 0; i < 4; i++) {
      ...
      // retry on contention with backoff
...
type HS<TCtor> = TCtor & { entityName: string; indexName: string; keyOf(state: IS<TCtor>): string; seedData?: ReadonlyArray<IS<TCtor>> };
...
export abstract class IndexedEntity<S extends { id: string }> extends Entity<S> {
  ...
  static async ensureSeed<TCtor extends CtorAny>(this: HS<TCtor>, env: Env): Promise<void> {
    ...
    // This is an upsert logic. It will overwrite existing data with the seed data.
    // This is useful for development to keep mock data in sync with code changes.
    console.log(`[SEED] Ensuring ${seeds.length} seed records for ${this.entityName}...`);
    // Save/upsert all seed entities
    await Promise.all(seeds.map(s => {
      const id = this.keyOf(s);
      const inst = new this(env, id);
      return inst.save(s);
    }));
    // Ensure all seed IDs are in the index
    const idx = new Index<string>(env, this.indexName);
    const seedIds = seeds.map(s => this.keyOf(s));
    await idx.addBatch(seedIds);
    console.log(`[SEED] Seeding complete for ${this.entityName}.`);
  }
